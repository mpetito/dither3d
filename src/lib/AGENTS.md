...
| `palette.ts`          | Per-layer (layer index → filament)  | `applyCyclic`, `applyBresenham`, `buildBresenhamLayerMap`, `buildTransitionLayerMap` |
...
Palette           — CyclicPalette { pattern[] } | BresenhamPalette { stops[] } | TransitionPalette { stops[], transitionWidth, maxCycleLength }
